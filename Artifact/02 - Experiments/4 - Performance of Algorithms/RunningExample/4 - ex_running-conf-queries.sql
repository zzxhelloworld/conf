/* Queries on the 2-CONF running example */

/* Conjunctive Queries*/
/* Q1: Which companies manage which venues */
select company, venue
from R3;
...
from R1 r, R2 s1, R2 s2
where r.event=s1.event and s1.event=s2.event and s1.venue<>s2.venue;

/* Union of Conjunctive Queries*/
/* Q4: List events managed by company "2" at venue "0" or company "5" at venue "5".*/
select r.event
from R1 r, R2 s
where r.event=s.event and r.company=2 and s.venue=0
union
select r.event
from R1 r, R2 s
where r.event=s.event and r.company=5 and s.venue=5;

/* Queries with Negation */
/* Q5: Which companies have never managed any event at more than one venue?*/
select distinct r.company 
from R1 r
where r.company not in (select distinct r1.company
from R1 r1, R2 s1, R2 s2
where r1.event=s1.event and s1.event=s2.event and s1.venue<>s2.venue);
...
from R1 r1, R2 s1
where r1.event=s1.event and r1.company=2 and s1.venue=0 and NOT EXISTS (
	select *
    from R1 r2, R2 s2
    where r2.event=s2.event and r2.company=2 and s2.venue=0 and NOT EXISTS (
		select *
        from R1 r3, R2 s3
        where r3.event=s3.event and r3.company=2 and s3.venue=0 and r3.event=r1.event and s3.time=s2.time)); 

/* Aggregation Queries */
/* Q7: How many times do companies run events? */
select r.event, r.company, count(distinct s.time) as number_of_times
from R1 r, R2 s
where r.event=s.event
group by r.event, r.company;

/* Q8: How many times do companies run events at the same venue? */
select r.event, r.company, s.venue, count(distinct s.time) as number_of_times
from R1 r, R2 s
where r.event=s.event
...
select r1.company 
from R1 r1, R2 s1, (select s.venue
from R1 r, R2 s
where r.event=s.event
group by s.venue
having count(distinct r.company)=1) t
where r1.event=s1.event and s1.venue=t.venue;

/* Q10: Which company has run the most events at the same venue? */
select distinct r.company, a2.venue, a2.num_of_events
from R1 r, (select r1.company, s1.venue, count(distinct r1.event) as num_of_events
                from R1 r1, R2 s1
                where r1.event=s1.event
